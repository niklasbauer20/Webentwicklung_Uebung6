    ...
    <form action="Projekte" method="post">
        <div class="form-group">
            <select class="form-select" name="id" aria-label="Default select example">
                <option selected
                        value="<?= (isset($_SESSION['projektid'])) ? $_SESSION['projektid'] : 0 ?>"><?= (isset($_SESSION['projektname'])) ? $_SESSION['projektname'] : '-bitte wählen-' ?></option>
            ...
            <button type="submit" name="btnAuswaehlen" id="btnAuswaehlen" class="btn btn-primary mt-4">Auswählen
            </button>
            <button type="submit" name="btnBearbeiten" id="btnBearbeiten" class="btn btn-primary mt-4">Bearbeiten
            </button>
    ...
    <div class="mt-4">
        <form action="Projekte/submit_edit" method="post">
            ...
            <input type="text" name="Name" id="Name" value="<?= isset($project['Name']) ? $project['Name'] : '' ?>"
                   class="form-control mt-1" placeholder="Projekt">

            <div class="mt-4">Projektbeschreibung:</div>
            <textarea name="Beschreibung" id="Beschreibung" class="form-control mt-1"
                      placeholder="Beschreibung"><?= isset($project['Beschreibung']) ? $project['Beschreibung'] : '' ?></textarea>
...
<script>
    document.getElementById("btnLoeschen").addEventListener("click", event =>{
        if (!confirm("Möchten Sie das Projekt wirklich loeschen?")) {
